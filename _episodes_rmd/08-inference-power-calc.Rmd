...
enough _power_, a term we are now going to define. If you are doing scientific 
...
```{r, echo=FALSE, fig.cap="Null distribution showing type I error as alpha"}
knitr::include_graphics("../fig/null-hypothesis.png")
```
...
these so we usually report power for several plausible values of <i>&Delta;, 
&sigma;<sub>X</sub>, &sigma;<sub>Y</sub></i> and various sample sizes.
...
> > 2. 
> >
> > 4. `se <- sd(dat.ns)/sqrt(N)`  
`qnorm(1-0.01/2)*se`
> > 5. `qt(1-0.01/2, df=2*N-2)*se` 
> > 8. `set.seed(1)`  
