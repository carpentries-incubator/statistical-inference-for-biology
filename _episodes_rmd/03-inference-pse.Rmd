...
With a sample size of 30, we will use the CLT. The CLT tells us that $\bar{X}$ or `mean(chow)` follows a normal distribution with mean <i>&mu;<sub>X</sub></i> or `mean(chowPopulation)` and standard error approximately  $s_X/\sqrt{N}$ or:
...
$\sqrt{N} (\bar{X}-<i>&mu;<sub>X</sub></i>) / s_X$ follows a normal distribution with mean 0 and
...
$\bar{X} &plusmn; 2 s_X / \sqrt{N}$, not <i>&mu;<sub>X</sub></i>, 
