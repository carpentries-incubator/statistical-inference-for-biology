...
With a sample size of 30, we will use the CLT. The CLT tells us that <i>X&#772;</i> or `mean(chow)` follows a normal distribution with mean <i>&mu;<sub>X</sub></i> or `mean(chowPopulation)` and standard error approximately  *s<sub>X</sub> /  &radic;N* or:
...
*&radic;N (X&#772;-<i>&mu;<sub>X</sub></i>) / s<sub>X</sub>* follows a normal distribution with mean 0 and
